...
std::map<std::string, int> ScaleSingleMap(
...
// Scales all the votes in the map (which is committee -> dist -> int),
// the scaling factors are a map of committee -> scaling factor
std::map<std::string, std::map<std::string, int>> ScaleVotesByParty(
...
std::map<std::string, std::map<std::string, int>> ScaleVotesByDistrict(
  ...
  // back again.
  return PivotMap(ScaleVotesByParty(PivotMap(source), scaling_factors));
}
